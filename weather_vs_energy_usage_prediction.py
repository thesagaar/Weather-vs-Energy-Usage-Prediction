...
# FINAL DASHBOARD + FINAL SUMMARY (COMBINED FOR sklearn NOTEBOOK)
# Replace your current plots cell with this full code
...
# ==========================================================
axs[0,0].scatter(x, y_actual, alpha=0.5, label="Observed Data")

axs[0,0].plot(
    x_sorted, y_pred_sorted,
    color="red", linewidth=3,
    label=f"ŷ = {a1:.2f}X + ({a0:.2f})"
)
...
axs[0,0].text(
    ...
    fontsize=10,
    bbox=dict(boxstyle="round", facecolor="#F5F5F5", edgecolor="black", alpha=0.95), color="black"
)
...
# ==========================================================
# 2. Sorted X vs Predicted Y
# ==========================================================
axs[0,1].plot(
    x_sorted, y_actual_sorted,
    linewidth=1.2, label="Actual (sorted)"
)
...
axs[1,1].axvline(rmse, color="cyan", linestyle=":", label="+RMSE")
axs[1,1].axvline(-rmse, color="cyan", linestyle=":", label="-RMSE")
...
print("="*65)
print("FINAL SUMMARY".center(65))
print("="*65)

print("\n--- Statistics for Appliances ---")
print(f" Mean: {df['Appliances'].mean():.4f} | Median: {df['Appliances'].median():.4f} | Mode: {df['Appliances'].mode()[0]:.4f} | StdDev: {df['Appliances'].std():.4f}")

print("\n--- Statistics for T_out ---")
print(f" Mean: {df['T_out'].mean():.4f} | Median: {df['T_out'].median():.4f} | Mode: {df['T_out'].mode()[0]:.4f} | StdDev: {df['T_out'].std():.4f}")

print("\n--- Statistics for RH_out ---")
print(f" Mean: {df['RH_out'].mean():.4f} | Median: {df['RH_out'].median():.4f} | Mode: {df['RH_out'].mode()[0]:.4f} | StdDev: {df['RH_out'].std():.4f}")

print("\n--- Statistics for Windspeed ---")
print(f" Mean: {df['Windspeed'].mean():.4f} | Median: {df['Windspeed'].median():.4f} | Mode: {df['Windspeed'].mode()[0]:.4f} | StdDev: {df['Windspeed'].std():.4f}")

print("\n--- Regression Model (Y = c + m1X1 + m2X2 + m3X3) ---")
print(f" Intercept (c) : {a0:.4f}")
print(f" Coeff T_out   : {a1:.4f}")
print(f" Coeff RH_out  : {a2:.4f}")
print(f" Coeff Wind    : {a3:.4f}")

print(f"\n Equation: Appliances = {a0:.2f} + ({a1:.2f} * T_out) + ({a2:.2f} * RH_out) + ({a3:.2f} * Windspeed)")

print("\n--- Error Metrics ---")
print(f" MAE  : {mae:.4f}")
print(f" RMSE : {rmse:.4f}")
print(f" R²   : {r2:.6f} ({r2*100:.2f}% accuracy)")